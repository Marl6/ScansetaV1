...
const formatMessageText = (text) => {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (numberedMatch) {
      ...
      listBuffer += `<div class="list-item"><span class="list-number">${numberedMatch[1]}</span><span class="list-content">${formatInlineText(numberedMatch[3])}</span></div>`;
      continue;
    }
    ...
    if (bulletMatch) {
      ...
      listBuffer += `<div class="list-item"><span class="list-bullet">•</span><span class="list-content">${formatInlineText(bulletMatch[2])}</span></div>`;
      continue;
    }
    ...
    // Regular paragraph
    formattedHtml += `<div class="ai-paragraph">${formatInlineText(line)}</div>`;
  }
  ...
  return <div dangerouslySetInnerHTML={{ __html: formattedHtml }} className="ai-content" />;
};
...
const EjieworxAIModal = ({ isOpen, onClose }) => {
  ...
  const handleSendMessage = async () => {
    ...
    setMessages([...newMessages, { text: 'Thinking...', sender: 'ai', id: thinkingId, isThinking: true }]);
    ...
    try {
      ...
      setMessages(prevMessages => 
        prevMessages.map(msg => 
          msg.id === thinkingId 
            ? { text: response.response || 'Sorry, I couldn\'t generate a response.', sender: 'ai' } 
            : msg
        )
      );
    } catch (error) {
      ...
      setMessages(prevMessages => 
        prevMessages.map(msg => 
          msg.id === thinkingId 
            ? { text: 'Sorry, there was an error processing your request. Please try again.', sender: 'ai' } 
            : msg
        )
      );
        ...
        <div className="confirm-modal-overlay">
          <div className="confirm-modal">
            ...
            <div className="confirm-modal-content">
              <p>Are you sure you want to start a new conversation? This will clear your current chat history.</p>
            </div>
            <div className="confirm-modal-footer">
              <button className="cancel-button" onClick={handleCancelNewChat}>Cancel</button>
              <button className="confirm-button" onClick={handleNewChat}>Yes, Start New</button>
            </div>
          </div>
        </div>
      )}
